...
class Upload_audio{
    ...
    function uploadMp3($asset_url,$fileName,$foldername){
        ...
        if (in_array($ext, $this->chkExt)) {
                ...
                move_uploaded_file($_FILES['uploadSong']['tmp_name'],$foldername.'/'.$asset_url_name);
                // copy($_FILES['uploadSong']['tmp_name'],$foldername.'/'.$asset_url_name);
                return array('success'=>1,'filepath'=> 'Success');
        }else{
